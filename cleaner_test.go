...
func TestCleaner(t *testing.T) {
	flushConn, err := OpenConnection("cleaner-flush", "tcp", "localhost:30001", 1, nil)
	assert.NoError(t, err)
	assert.NoError(t, flushConn.stopHeartbeat())
	assert.NoError(t, flushConn.flushDb())

	conn, err := OpenConnection("cleaner-conn1", "tcp", "localhost:30001", 1, nil)
	assert.NoError(t, err)
	...
	cleanerConn, err := OpenConnection("cleaner-conn", "tcp", "localhost:30001", 1, nil)
	assert.NoError(t, err)
